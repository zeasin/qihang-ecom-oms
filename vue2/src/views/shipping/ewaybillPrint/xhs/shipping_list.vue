      ...
      <el-col :span="1.5">
        <el-select size="mini"  v-model="form.accountId" placeholder="请选择电子面单账户" clearable>
          <el-option
            ...
            <span style="float: right; color: #8492a6; font-size: 13px" >{{item.branchName}}</span>
          </el-option>
        </el-select>
      </el-col>
      <el-col :span="1.5">
...
import {pushWaybillPrintSuccess,pushShipSend} from "@/api/wei/ewaybill";
import {listOrder, getOrder } from "@/api/order/order";
import {listAllMerchant} from "@/api/shop/merchant";
import {getWaybillCode,getWaybillAccountList} from "@/api/xhs/ewaybill";
...
export default {
  ...
  methods: {
    ...
    openWs() {
      const ws = new WebSocket('ws://127.0.0.1:10818');
      ws.onopen = () => {
      ...
      ws.onerror = function(error) {
        obj.msgError("打印组件连接失败！请安装并启动小红书打印组件！");
