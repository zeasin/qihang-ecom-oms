...
export default {
  ...
  created() {
    listAllMerchant({pageNum: 1, pageSize: 1000}).then(resp => {
      ...
      listShop({merchantId: this.queryParams.merchantId,type:this.queryParams.shopType}).then(response => {
        ...
        //   this.shopList.push({name:'总部销售订单',id:0})
        // }
        this.shopList.push(...response.rows)
        // if(this.shopList&&this.shopList.length>0){
        //   this.queryParams.shopId = this.shopList[0].id
        // }
        getWaybillAccountList({supportOffline:1}).then(response => {
  ...
  methods: {
    ...
    handleGetEwaybillCode() {
      ...
      if (ids && ids.length > 0 && this.form.accountId) {
        console.log("===========取号并弹出结果=========")
