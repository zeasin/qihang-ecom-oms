    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="108px">
      ...
      <el-form-item label="类型" prop="refundType">
        <el-select v-model="queryParams.refundType" placeholder="请选择类型" clearable @change="handleQuery">
          ...
          <el-option label="商品补发" value="80" ></el-option>
        </el-select>
      </el-form-item>
...
<!--      <el-table-column label="ID" align="center" prop="id" />-->
      <el-table-column label="售后单号" align="left" prop="refundNum" >
...
<script>
import {listSalesAfter,salesAfterProcessing} from "@/api/cloud_warehouse/salesAfter";

export default {
  name: "WarehouseAfterSale",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSalesAfter(this.queryParams).then(response => {
    ...
    submitForm(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          salesAfterProcessing(this.form).then(response => {
