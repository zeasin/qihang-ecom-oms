...
<!--      <el-table-column label="订单ID" align="center" prop="id" />-->
      <el-table-column label="订单号" align="left" prop="orderNum" width="200px">
        ...
        >{{scope.row.orderNum}} </el-button><i class="el-icon-copy-document tag-copy" :data-clipboard-text="scope.row.orderNum" @click="copyActiveCode($event,scope.row.orderNum )" ></i><br/>
      ...
      <el-table-column label="商品明细" align="center" width="900px" >
        ...
        <template slot-scope="scope" >
          <el-table :data="scope.row.itemList" :show-header="false" :cell-style="{border:0 + 'px' }"  :row-style="{border:0 + 'px' }" >
            ...
            <el-table-column label="商品数量" align="center" prop="quantity" width="70px">
              <template slot-scope="scope">
...
<!--        </template>-->
<!--      </el-table-column>-->
      <el-table-column label="订单状态" align="center" prop="sendStatus" >
        <template slot-scope="scope">
          <el-tag v-if="scope.row.orderStatus === 1">待发货</el-tag>
      ...
      <el-table-column label="发货状态" align="center" prop="sendStatus" >
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.sendStatus === 1">部分发货</el-tag>
          <el-tag v-if="scope.row.sendStatus === 2">全部发货</el-tag>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          ...
          >详情</el-button>
            <el-button style="padding-right: 6px;padding-left: 6px;"
              size="mini" v-if="scope.row.sendStatus === 0||scope.row.sendStatus === 1"
              type="success"
              icon="el-icon-truck"
              plain
...
<!--            </el-descriptions>-->
        ...
        <el-tab-pane label="商品列表" name="orderItems" lazy>
          <el-table :data="form.itemList"  style="margin-bottom: 10px;">
            ...
            <el-table-column label="数量" align="center" prop="quantity" width="80px">
              <template slot-scope="scope">
                <el-tag size="small" type="danger">{{scope.row.quantity}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="退款状态" align="left" prop="refundStatus" width="100">
              <template slot-scope="scope">
            ...
            <el-table-column label="发货状态" prop="sendStatus" width="100">
              <template slot-scope="scope">
                <el-tag v-if="scope.row.sendStatus === 0">待发货</el-tag>
                <el-tag v-if="scope.row.sendStatus === 1">部分发货</el-tag>
                <el-tag v-if="scope.row.sendStatus === 2">全部发货</el-tag>
...
<!--        </el-tab-pane>-->
    ...
    <el-dialog title="手动发货" :visible.sync="shipOpen" width="1400px" append-to-body>
      <el-form ref="form" :model="form"  :rules="rules" label-width="80px">
        <el-table :data="form.itemList"  style="margin-bottom: 10px;">
          ...
          <el-table-column label="待发货数量" align="center" prop="unshippedQuantity" width="150px">
            <template slot-scope="scope">
              <el-tag size="small" type="danger">{{scope.row.unshippedQuantity}}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="发货状态" prop="sendStatus" width="100">
            <template slot-scope="scope">
          ...
          <el-table-column label="发货数量" align="left" prop="unshippedQuantity" width="80">
            <template slot-scope="scope">
              <el-input v-if="scope.row.sendStatus === 0||scope.row.sendStatus === 1" v-model="scope.row.shipQuantity" placeholder="发货数量" style="width: 60px" />
            </template>
          </el-table-column>
        ...
        <el-form-item label="快递单号" prop="logisticsCode">
          <el-input v-model="form.logisticsCode" placeholder="请输入快递单号" style="width: 300px"/>
        </el-form-item>

      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="shippingSubmit">确认发货</el-button>
...
import { logisticsManualSend } from '@/api/vms/ship'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        logisticsCode: [{ required: true, message: "请填写快递单号", trigger: "change" }],
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    handleShip(row){
      this.form.id = row.id;
      this.form.logisticsCompany = null;
      this.form.itemList = row.itemList
      this.form.itemList.forEach(x=>{
        x.shipQuantity = x.unshippedQuantity
      })
      const shopType = this.orderList.filter(x=>x.id === row.id).shopType;
      getFavoriteList({status:1,shopType:shopType}).then(response => {
    ...
    shippingSubmit(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          console.log("手动发货====提交=====",this.form)
          logisticsManualSend(this.form).then(resp=>{
            this.shipOpen=false
