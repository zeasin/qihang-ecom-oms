...
import {getJdCloudWarehouseList,listJdCloudWarehouseShipper,addJdlShipper} from "@/api/cloud_warehouse/jd";
import {updateJdlBaseInfo} from "@/api/cloud_warehouse/jdl_isc";
import {pullJkyShops} from "@/api/cloud_warehouse/jky";
import {listCloudWarehouseShop} from "@/api/cloud_warehouse";
...
export default {
  ...
  methods: {
    ...
    handlePull(){
      pullJkyShops({warehouseId:this.queryParams.warehouseId}).then(resp=>{
        if(resp.code===200){
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          addJdlShipper(this.form).then(response => {
